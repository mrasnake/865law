...
// 1. Init common functions on document ready
    ...
    $(function() {
        ...
        live_analytics_page_onload.init();

        // main header
        live_analytics_main_header.init();

        // main sidebar
        live_analytics_main_sidebar.init();
        // secondary sidebar
        live_analytics_secondary_sidebar.init();

        // top bar
        live_analytics_top_bar.init();

        // page heading
        live_analytics_page_heading.init();

        // material design
        live_analytics_md.init();

        // forms
        live_analytics_forms.init();

        // truncate text helper
        live_analytics_helpers.truncate_text($('.truncate-text'));

        // full screen
        live_analytics_helpers.full_screen();
...
// 3. Common functions & variables
    ...
    live_analytics_page_onload = {
        init: function() {
            $window.load(function(){
                // fire functions on window load
                live_analytics_helpers.hierarchical_show();
                live_analytics_helpers.hierarchical_slide();
            });
        }
    };

    // 3.3 page content
    live_analytics_page_content = {
    ...
    // 3.4 forms
    live_analytics_forms = {
        init: function() {
            live_analytics_forms.textarea_autosize();
            live_analytics_forms.select_elements();
            live_analytics_forms.switches();
        },
    ...
    live_analytics_main_sidebar = {
        init: function() {
            if($sidebar_main.length) {
                // check which sidebar should be loaded (regular or mini)
                if (!$body.hasClass('sidebar_mini') && localStorage.getItem("live_analytics_sidebar_mini") === null) {
                    $sidebar_main_toggle.on('click', function(e) {
                        e.preventDefault();
                        ( $body.hasClass('sidebar_main_active') || ($body.hasClass('sidebar_main_open') && $window.width() >= 1220) ) ? live_analytics_main_sidebar.hide_sidebar() : live_analytics_main_sidebar.show_sidebar();
                    });
                    ...
                    $document.on('click keyup', function(e) {
                        if( $body.hasClass('sidebar_main_active') && $window.width() < 1220 ) {
                            if (
                                ...
                                live_analytics_main_sidebar.hide_sidebar();
                            }
                        }
                    });

                    // custom scroller
                    live_analytics_helpers.custom_scrollbar($sidebar_main);

                    if( $body.hasClass('sidebar_main_active') && $window.width() < 1220 ) {
                        live_analytics_page_content.hide_content_sidebar();
                    } else {
                        live_analytics_page_content.show_content_sidebar();
                    }
                    // menu
                    live_analytics_main_sidebar.sidebar_menu();
                    // swipe to open (touch devices)
                    live_analytics_main_sidebar.swipe_open();
                } else {

                    // small sidebar
                    live_analytics_main_sidebar.mini_sidebar();
                ...
                // language switcher
                live_analytics_main_sidebar.lang_switcher();
            }
        },
            ...
            if( $body.hasClass('sidebar_main_swipe') && Modernizr.touch) {
                ...
                mc.on("swiperight", function() {
                    if (!$body.hasClass('sidebar_main_active')) {
                        live_analytics_main_sidebar.show_sidebar();
                    }
                });
    ...
    live_analytics_secondary_sidebar = {
        init: function() {
            if($sidebar_secondary.length) {
                ...
                $sidebar_secondary_toggle.on('click', function (e) {
                    e.preventDefault();
                    $body.hasClass('sidebar_secondary_active') ? live_analytics_secondary_sidebar.hide_sidebar() : live_analytics_secondary_sidebar.show_sidebar();
                });
                ...
                $document.on('click keydown', function(e) {
                     if(
                         ...
                         live_analytics_secondary_sidebar.hide_sidebar();
                     }
                });
                ...
                // custom scroller
                live_analytics_helpers.custom_scrollbar($sidebar_secondary);

                // chat section
                live_analytics_secondary_sidebar.chat_sidebar();
    ...
    // main header
    live_analytics_main_header = {
        init: function() {
            live_analytics_main_header.search_activate();
            live_analytics_main_header.search_autocomplete();
        },
        search_activate: function() {

            $('#main_search_btn').on('click',function(e) {
                e.preventDefault();
                live_analytics_main_header.search_show();
            });
            ...
            $(document).on('click keydown', function(e) {
                if( !$body.hasClass('main_search_persistent') && $body.hasClass('main_search_active') ) {
                    if (
                        ...
                        live_analytics_main_header.search_hide();
                    }
                }
            });
    ...
    live_analytics_md = {
        init: function() {
            live_analytics_md.inputs();
            live_analytics_md.checkbox_radio();
            live_analytics_md.card_fullscreen();
            live_analytics_md.card_expand();
            live_analytics_md.card_overlay();
            live_analytics_md.card_single();
            live_analytics_md.card_panel();
            live_analytics_md.card_progress();
            live_analytics_md.list_outside();
            // FAB transitions
            live_analytics_md.fab_speed_dial();
            live_analytics_md.fab_toolbar();
            live_analytics_md.fab_sheet();
            live_analytics_md.wave_effect();
        },
        ...
        card_panel: function() {
            ...
            $('.md-card-close').on('click',function(e) {
                ...
                live_analytics_md.card_show_hide(thisCard,undefined,removeCard)
            });
        ...
        list_outside: function() {
            ...
            if($md_list_outside_wrapper && $body.hasClass('header_double_height')) {
                ...
                live_analytics_helpers.custom_scrollbar($md_list_outside_wrapper);
            }
        },
        inputs: function(parent) {
            ...
            $mdInput.each(function() {
                if(!$(this).closest('.md-input-wrapper').length) {
                    ...
                    live_analytics_md.update_input($this);
                }
                $body
        ...
        color_picker: function(object,pallete) {
            if(object) {
                var cp_id = randID_generator(),
                    ...
                    cp_wrapper = $('<div class="cp_live_analytics" id="'+cp_id+'"/>');
                ...
                $body.on('click', '#'+cp_id+' span',function() {
                    $(this)
                        ...
                        .end()
                        .closest('.cp_live_analytics').find('input').val($(this).attr('data-color'));
                });
